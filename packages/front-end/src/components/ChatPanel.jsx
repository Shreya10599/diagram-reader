...
// The two forms the backend's hardcoded FORM_SCHEMAS supports (see
// app/form_schemas.py) — kept in sync manually since there's no schema
// listing endpoint yet.
const FORM_OPTIONS = [
  { formType: 'liheap', label: 'LIHEAP energy assistance worksheet' },
  { formType: 'stock_basis', label: 'Inherited stock cost-basis worksheet' },
]
...
export default function ChatPanel({
  ...
  // Runs the form-filling call and drops the resulting worksheet into the
  // chat as its own message type (see the 'worksheet' render branch
  // below) — same turn-taking pattern as handleSend, just with a
  // structured result instead of plain text.
  const handleFillForm = async (formType, formLabel) => {
    setMessages((prev) => [...prev, { id: nextId(), role: 'user', text: `Fill out the ${formLabel}` }])
    setIsFillingForm(true)
    setIsWaitingForAnswer(true)

    try {
      const result = await onFillForm(formType)
      setMessages((prev) => [
        ...prev,
        {
          id: nextId(),
      ...
      const errorMsg = `Sorry, something went wrong filling that worksheet: ${err.message || err}`
      setMessages((prev) => [...prev, { id: nextId(), role: 'assistant', text: errorMsg }])
      speak(errorMsg)
    } finally {
  ...
  // Worksheet fields start out as whatever the backend computed/flagged,
  // but that's not something the person can actually use — manual fields
  // (name, account number, ...) have no value at all, and needs_review
  // values may need correcting. Edits are kept per-message in an
  // `editedValues` map (field_id -> string) rather than mutating `fields`
  // directly, so the original backend result (status/reason) stays intact
  // for the download/reference while the input reflects what was typed.
  const handleWorksheetFieldChange = (msgId, fieldId, newValue) => {
    setMessages((prev) =>
      prev.map((m) =>
        m.id === msgId
          ? { ...m, editedValues: { ...(m.editedValues || {}), [fieldId]: newValue } }
          : m
      )
    )
  }

  // Only way to actually "have" the worksheet outside this chat — exports
  // the current field values (including anything typed into the manual/
  // needs_review inputs) as a plain-text file the person can open, save,
  // or print. Still just a worksheet: the file says as much up top, and
  // nothing here submits it anywhere.
  const handleDownloadWorksheet = (msg) => {
    const formLabel = FORM_OPTIONS.find((o) => o.formType === msg.formType)?.label ?? msg.formType
    const lines = [
      `${formLabel} — worksheet`,
      'Generated by Diagram Reader. Review every value before using it — nothing here was submitted anywhere automatically.',
      '',
    ]
    msg.fields.forEach((f) => {
      const value = msg.editedValues?.[f.field_id] ?? (f.value != null ? String(f.value) : '')
      const unitSuffix = f.unit && value ? ` ${f.unit}` : ''
      lines.push(`${f.label}: ${value ? `${value}${unitSuffix}` : '(not filled in)'}`)
      if (f.status === 'needs_review') lines.push(`  Double check — ${f.reason}`)
      if (f.status === 'manual_required') lines.push('  (fill in yourself — not on the chart)')
    })
    ...
    a.href = url
    a.download = `${msg.formType}-worksheet.txt`
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
    URL.revokeObjectURL(url)
    speak('Worksheet downloaded')
  }
  ...
  return (
    <div className="chat-panel">
      ...
      {/* Shown once there's a chart to work from — clicking a form re-runs
          extraction with a task scoped to that form's needed figure (see
          App.jsx's handleFillForm), it doesn't reuse computedAnswer from
          the original analysis. This is a worksheet, not an auto-submit —
          see the caution line rendered inside each worksheet message below. */}
      {hasChart && (
        <div className="form-fill-actions" role="group" aria-label="Fill a worksheet from this chart">
          <span className="form-fill-label">Fill a worksheet from this chart:</span>
          {FORM_OPTIONS.map((opt) => (
            <button
              key={opt.formType}
              type="button"
              className="secondary-btn form-fill-btn"
              disabled={isFillingForm}
              onClick={() => handleFillForm(opt.formType, opt.label)}
            >
              {opt.label}
            </button>
          ))}
        ...
        {messages.map((msg) => (
          ...
          >
            {msg.role === 'suggestion' ? (
              ...
              <div className="message-bubble worksheet-bubble">
                <span className="message-role">Worksheet — review before you copy these in</span>
                <span className="worksheet-caution">
                  Nothing here is submitted anywhere automatically — double-check every value
                  against the original chart or your records before using it.
                </span>
                <ul className="worksheet-fields">
                  {msg.fields.map((f) => {
                    const fieldInputId = `worksheet-field-${msg.id}-${f.field_id}`
                    const currentValue =
                      msg.editedValues?.[f.field_id] ?? (f.value != null ? String(f.value) : '')
                    return (
                      <li key={f.field_id} className={`worksheet-field worksheet-field-${f.status}`}>
                        ...
                        <input
                          id={fieldInputId}
                          type="text"
                          className="worksheet-field-input"
                          value={currentValue}
                          placeholder={
                            f.status === 'manual_required' ? 'Fill in yourself — not on the chart' : ''
                          }
                          onChange={(e) =>
                            handleWorksheetFieldChange(msg.id, f.field_id, e.target.value)
                          }
                        />
                        {f.status === 'needs_review' && (
                          <span className="worksheet-field-note">⚠ Double check — {f.reason}</span>
                        )}
                      </li>
                    )
                  })}
                ...
                <button
                  type="button"
                  className="control-btn worksheet-download-btn"
                  onClick={() => handleDownloadWorksheet(msg)}
                >
                  ⬇ Download worksheet
                </button>
                <span className="worksheet-summary">{msg.summary}</span>
              </div>
            ) : (
